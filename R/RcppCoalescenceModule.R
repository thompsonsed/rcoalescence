...
#' setting up, running and analysing TreeSimulations. The general process is
#' \itemize{
...
SpatialTreeSimulation <-
  setRcppClass(
    ...
    methods = list(
      setDispersalParameters = function(sigma,
      ...
      applySpeciationRates = function(speciation_rates = NA,
        ...
        ._applySpeciationRates(output_file,
                               use_spatial,
                               sample_file,
                               use_fragments,
                               times_list)
...
#' #' Applying a metacomunity to a completed Simulation
#' #' @name Metacommunity
#' #' @export Metacommunity
#' #' @description Applies a metacommunity to a completed Simulation object.
#' #'
#' #' Requires a completed simulation from one of the
#' #'  \link[=NeutralTreeSimulation]{four Simulation classes}.
#' #'
#' Metacommunity <- setRcppClass(
#'   "Metacommunity",
#'   "RMetacommunity",
#'   module = "coalescenceModule",
#'   methods = list(
...
#'         use_fragments <- substr(as.character(use_fragments), 1, 1)
#'       }
#'       ._setup(database, use_spatial, sample_file, use_fragments)
#'       if (anyNA(speciation_rates))
#'       {
#'         stop("Speciation rates cannot be NA.")
#'       }
#'       if (is.vector(speciation_rates))
#'       {
#'         for (spec_rate in speciation_rates)
...
#'       else
#'       {
#'         .addSpeciationRate(speciation_rates)
#'       }
#'       addTimes(times)
#'       addMetacommunityParameters(
#'         metacommunity_option,
...
#'         metacommunity_external_reference
#'       )
#'       addProtractedParameters(min_speciation_gens, max_speciation_gens)
#'     },
#'
...
#'     },
#'
#'     addMetacommunityParameters = function(metacommunity_option = NA,
#'                                           metacommunity_size = NA,
#'                                           metacommunity_speciation_rate =
#'                                             NA,
#'                                           metacommunity_external_reference =
...
#'       },
#'
#'     addProtractedParameters = function(min_speciation_gens,
#'                                        max_speciation_gens)
#'     {
