---
title: "Simulating species abundance distributions on an archipelago"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Simulating species abundance distributions on an archipelago}
...
dispersal_probabilities <- island_dispersal %>%
  pivot_longer(names_to = "destination", values_to = "probability", cols = c(A, B, C, D)) %>%
...
knitr::kable(species_island_A %>% head(10))
```

The community that most closely approximates the known sample is that with community reference `r best_community_reference$community_reference`, which corresponds to a speciation rate of `r scales::scientific(best_community_reference$speciation_rate)`. It produces a mean species richness of `r best_community_reference$mean_total_species`. We can now use this reference simulation to calculate the dynamics for the rest of the community. 
...
knitr::kable(island_species_abundances %>%
             ...
             label="Number of individuals and species richness for each island.")
```
